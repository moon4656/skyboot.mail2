...
class Mail(Base):
    ...
    mail_id = Column(String(50), unique=True, index=True, default=generate_mail_id, comment="메일 ID (년월일_시분초_uuid)")
    org_id = Column(String(36), ForeignKey("organizations.org_id"), nullable=False, comment="소속 조직 ID")
    ...
    priority = Column(SQLEnum(MailPriority), default=MailPriority.NORMAL, comment="우선순위")
    status = Column(SQLEnum(MailStatus), default=MailStatus.DRAFT, comment="메일 상태")
    is_draft = Column(Boolean, default=False, comment="임시보관함 여부")
    
    # 메일 헤더 정보
    message_id = Column(String(255), unique=True, comment="메시지 ID")
    ...
    folder_relations = relationship("MailInFolder", back_populates="mail")
...
class MailRecipient(Base):
    ...
    id = Column(BigInteger, primary_key=True, index=True)
    mail_id = Column(String(50), ForeignKey("mails.mail_id"), nullable=False, comment="메일 ID")
    recipient_id = Column(String(36), ForeignKey("mail_users.user_uuid"), nullable=False, comment="수신자 UUID")
...
class MailInFolder(Base):
    ...
    # 관계 설정
    mail = relationship("Mail", back_populates="folder_relations")
